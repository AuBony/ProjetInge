...
# WITH SAME MIX OF DATA FOR THE 2 METHODS ----
...
# finding optimal couple for amplitude

param <- expand.grid(amp_lim = seq(0.1, 1, by = 0.1), 
                     diff_lim = seq(20000, 60000, by = 2000))
param$mse <- rep(0, nrow(param))
for (i in 1:nrow(param)){
  count <- c()
  print(paste('amp: ',param$amp_lim[i], 'diff: ', param$diff_lim[i]))
  for (k in 1:nb_samples){
    count <- c(count, count_peaks_m(left_m = left_l[[k]],
                                    amp_lim = param$amp_lim[i],
                                    diff_lim = param$diff_lim[i]))
  }
  param$mse[i] <- mse(count, actual_rand)
...
p <- ggplot(param, aes(x = amp_lim, y = diff_lim, fill = log(mse))) +
  ...
  scale_fill_gradient(low = 'springgreen3', high = 'blue') +
  scale_x_continuous(breaks = seq(0.1, 1, by = 0.1)) +
  scale_y_continuous(breaks = seq(20000, 60000, by = 2000)) +
  theme_minimal()
ggplotly(p)
cat('mse : ', min(param$mse))
cat('amp_opt: ', param$amp_lim[which.min(param$mse)])
cat('diff_opt: ', param$diff_lim[which.min(param$mse)])

# finding optimal couple for ML

param <- expand.grid(prob_lim = seq(0.1, 1, by = 0.1), 
                     diff_lim = seq(1000, 60000, by = 2000))
param$mse <- rep(0, nrow(param))
for (i in 1:nrow(param)){
  count <- c()
  print(paste('prob: ',param$prob_lim[i], 'diff: ', param$diff_lim[i]))
...
ggplotly(p)
cat('mse : ', min(param$mse))
cat('prob_opt: ', param$prob_lim[which.min(param$mse)])
cat('diff_opt: ', param$diff_lim[which.min(param$mse)])
...
count6 <- c()
for (k in 1:nb_samples){
  count5 <- c(count5, count_peaks_m(left_m = left_l[[k]], 
                                    diff_lim = diff_lim, 
                                    amp_lim = amp_lim))
  count6 <- c(count6, count_peaks_ML_m(prob_lim = prob_lim,
                                       diff_lim = diff_lim,
                                       pred_rf = pr[[k]]))
}
count5
...
# Amplitude prediction vs actual graph
