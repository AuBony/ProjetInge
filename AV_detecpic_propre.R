...
# COUNTING WITH AMP_LIM=0.4 AND DIFF_LIM=18500 ----

# dataframe with all the file names
fich <- data.frame(filename = list.files(paste0(path,'cleanwav'))) 
# dataframe with file names and number of breaks
actual <- read.table(paste0(path, 'nb_bk.csv'), 
                     sep = ';', dec = '.', header = TRUE)
actual$filename <- as.factor(actual$filename)
summary(actual)
# list of the wav files
wavlist <- apply(fich, 1, FUN = imp_norm, path_wav = paste0(path,'cleanwav/'))

# fixing the 2 parameters
amp_lim = 0.4
diff_lim = 18500
...
# FINDING OPTIMAL COUPLE OF PARAMETERS ----

param <- expand.grid(amp_lim = seq(0.1, 1, by = 0.1), 
                     diff_lim = seq(1000, 50000, by = 100))
...
p <- ggplot(param, aes(x = amp_lim, y = diff_lim, fill = log(mse))) +
  ...
  scale_y_continuous(breaks = seq(1000, 50000, by = 5000)) +
  theme_minimal()
ggplotly(p)
min(param$mse)
...
# USING THE OPTIMAL COUPLE TO COUNT ----
...
for (k in 1:nrow(actual)){
  print(k)
  count2 <- c(count2, count_peaks(wav = wavlist[[k]],
                                 amp_lim = amp_lim,
                                 diff_lim = diff_lim))
}
count2
mse(count2, actual$nb_bk)
length(which(count2 == actual$nb_bk))

df2 <- data.frame(actual = actual$nb_bk, prediction = count2)
df2 <- df2 %>% add_count(actual, prediction)
m <- max(actual$nb_bk)+1

title2 <- paste0('Number of breaks, predicted vs actual \n for amp_lim = ', amp_lim,
                 ', diff_lim = ', diff_lim)
ggplot(df2, aes(x = actual, y = prediction)) +
...
p <- ggplot(param[1:496,], aes(x = amp_lim, y = diff_lim, fill = log(mse))) +
  geom_tile() +
  scale_fill_gradient(low = 'springgreen3', high = 'blue') +
  scale_x_continuous(breaks = seq(0.1, 1, by = 0.1)) +
...
pred_pts[[7]] %>% ggplot(aes(x=time, y=prob_1)) +
  geom_line() +
  theme_minimal()
plot(wavlist[[7]])
...
for (k in 1:nrow(actual)){
  print(k)
  count <- c(count, count_peaks_ML(diff_lim = diff_lim, 
                                   prob_lim = prob_lim,
                                   pred_rf = pred_pts[[k]]))
}
count
